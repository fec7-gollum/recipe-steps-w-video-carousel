...
((n = 100) => {
  for (let i = 2; i <= n; i += 1) {
    const sqlString = `INSERT INTO recipes (name) VALUES ('${faker.lorem.words(faker.random.number({ min: 1, max: 5 }))}');`;
    dbQuery(sqlString);
    for (let j = 1; j <= faker.random.number({ min: 5, max: 15 }); j += 1) {
      const hasVideos = faker.random.number(1);
      const sqlString1 = `INSERT INTO steps (recipes_id, number, text, hasVideos) \
      VALUES (${i}, ${j}, '${faker.lorem.sentences(faker.random.number({ min: 10, max: 20 }))}', ${hasVideos} );`;
      dbQuery(sqlString1);
      if (hasVideos) {
        const sqlString2 = 'SELECT id FROM steps ORDER BY id DESC LIMIT 1;';
        dbQuery(sqlString2, (result) => {
          const stepId = result[0].id;
          for (let k = 1; k <= faker.random.number({ min: 1, max: 2 }); k += 1) {
            const sqlString3 = `INSERT INTO videos (steps_id, url) VALUES (${stepId}, '${videoUrls[faker.random.number({ min: 0, max: videoUrls.length - 1})]}');`;
            dbQuery(sqlString3);
          }
        });
      }
